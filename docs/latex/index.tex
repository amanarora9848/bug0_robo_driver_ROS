...
In addition to these, in order to show the implementation of a R\+OS Service-\/\+Client, a node\+: ({\ttfamily disp\+\_\+goals\+\_\+node}) has also been created, that calls the Service Server node {\ttfamily goal\+\_\+info\+\_\+srv\+\_\+node} and prints the number of goals reached and cancelled.\hypertarget{index_autotoc_md1}{}\doxysection{Requirements}\label{index_autotoc_md1}
The overall project has been well tested using R\+OS Noetic and requires R\+Viz and Gazebo for simulation and physics.\hypertarget{index_autotoc_md2}{}\doxysection{Directory Structure \mbox{[}\+Package\+: unige\+\_\+rt1\+\_\+assignment2\mbox{]}}\label{index_autotoc_md2}
...
\end{DoxyCode}
\hypertarget{index_autotoc_md3}{}\doxysection{Running the full simulation\+:}\label{index_autotoc_md3}
\hypertarget{index_autotoc_md4}{}\doxysubsubsection{Update and upgrade packages\+:}\label{index_autotoc_md4}

\begin{DoxyCode}{0}
\DoxyCodeLine{sudo apt update \&\& sudo apt upgrade}
\end{DoxyCode}
\hypertarget{index_autotoc_md5}{}\doxysubsubsection{First, we need to build a catkin workspace for the project. Navigate to preferred directory, then\+:}\label{index_autotoc_md5}

\begin{DoxyCode}{0}
\DoxyCodeLine{mkdir -\/p bug0robo\_catkin\_workspace/src/}
\DoxyCodeLine{cd bug0robo\_catkin\_workspace/src/}
\end{DoxyCode}
\hypertarget{index_autotoc_md6}{}\doxysubsubsection{Initialize the catkin workspace\+:}\label{index_autotoc_md6}

\begin{DoxyCode}{0}
\DoxyCodeLine{catkin\_init\_workspace}
\end{DoxyCode}
\hypertarget{index_autotoc_md7}{}\doxysubsubsection{Now clone this repo and the action server repo (assuming user has ssh-\/auth setup)\+:}\label{index_autotoc_md7}
...
\end{DoxyCode}
\hypertarget{index_autotoc_md8}{}\doxysubsubsection{Navigate back to base directory $<$tt$>$bug0robo\+\_\+catkin\+\_\+workspace$<$/tt$>$}\label{index_autotoc_md8}

\begin{DoxyCode}{0}
\DoxyCodeLine{cd ..}
\end{DoxyCode}
\hypertarget{index_autotoc_md9}{}\doxysubsubsection{Run $<$tt$>$catkin\+\_\+make$<$/tt$>$ to build the package\+:}\label{index_autotoc_md9}

\begin{DoxyCode}{0}
\DoxyCodeLine{catkin\_make}
\end{DoxyCode}
\hypertarget{index_autotoc_md10}{}\doxysubsubsection{Source $<$tt$>$setup.\+bash$<$/tt$>$ to set the environment variables used by R\+OS}\label{index_autotoc_md10}

\begin{DoxyCode}{0}
\DoxyCodeLine{source devel/setup.bash}
\end{DoxyCode}
\hypertarget{index_autotoc_md11}{}\doxysubsubsection{Launch the complete simulation. The launch file for this can be found in $<$tt$>$/src/unige\+\_\+rt1\+\_\+assignment2/launch/complete\+\_\+sim.\+launch$<$/tt$>$ relative to the current diretory\+:}\label{index_autotoc_md11}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{index_autotoc_md12}{}\doxysection{A custom message for robot position and velocity}\label{index_autotoc_md12}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{index_autotoc_md13}{}\doxysubsection{\`{}\`{}\`{}}\label{index_autotoc_md13}
...
\end{DoxyItemize}\hypertarget{index_autotoc_md16}{}\doxysubsection{Helpful references / documentation / answers\+:}\label{index_autotoc_md16}
